...
class Demo:
    ...
    def _load_memory(self):
        try:
            ...
            self.memory = {"long_term": [], "facts": [], "traits": [], "use_tools": []}
    ...
    def _manage_context(self, new_message):
        ...
        # 加入经验记忆
        memory_as_text = json.dumps(self.memory, ensure_ascii=False)
        self.conversation_history.insert(1, {
            "role": "system",
            "content": f"以下为你的长期记忆，请在推理与回答时参考：\n{memory_as_text}"
        })
            ...
            while True:
                ...
                for tool_call in tool_calls:
                    ...
                    if tool_name in self.available_functions:
                        ...
                        self.conversation_history.append(tool_message)
            # 总结经验（错的别再错，对的记住）
            self._update_memory_from_dialogue(message, final_response)
            # 返回调用结果
    ...
    # 判断是否需要记住什么
    def _update_memory_from_dialogue(self, user_msg, assistant_msg):
        print(user_msg)
        print(assistant_msg)
        ...
        """
        # 让模型总结对话进行判断（不用流式，也不用调用工具，直接对上一轮对话作判断）
        res = ollama.chat(
        ...
        try:
            ...
            # 保存经验
            for key in ["long_term", "facts", "traits", "use_tools"]:
                if key in data:
    ...
    def _get_function_parameters(self, function):
        ...
        # 读取指定文件
        elif function.__name__ == 'read_file':
            return {
                "type": "object",
                "properties": {
                    "file_path": {
